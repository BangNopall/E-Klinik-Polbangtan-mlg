function selectConfigs() {
    return {
        ...
        fetchOptions() {
            this.options = {
                // disini buat ngatur fetch datanya, awm iso gae data json atau gimanapun itu dah
                results: [],
            };
            const url = document.getElementById("url_satuan_obat").textContent;
            fetch(url)
                .then((response) => {
                    if (!response.ok) {
        ...
        onOptionClick(index) {
            ...
            this.selectOption();
            document.getElementById("satuanID").value = this.selected.id;
        },
        selectOption() {
            if (!this.isOpen()) {
                return;
