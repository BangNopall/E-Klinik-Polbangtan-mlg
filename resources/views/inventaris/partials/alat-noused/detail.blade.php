<x-app-layout>
    {{-- Main Content --}}
    @include('inventaris.partials.modals.alat-noused.deposit', ['data' => $data])
    @include('inventaris.partials.modals.alat-noused.withdraw', ['data' => $data])
    @empty($data->kategori_id)
        @include('inventaris.partials.modals.alat-noused.notkategori', [
            'data' => $data,
        ])
    @endempty
        ...
        <div class="flex flex-col lg:flex-row gap-5">
            <div class="w-full lg:w-[500px]">
                        ...
                        <div class="flex mr-6 items-center">
                            ...
                            <span class="ml-1">
                                @isset($data->kategori_id)
                                    {{ $data->KategoriConsumable->nama_kategori }}
                                @endisset
                            </span>
                        </div>
                        <div class="flex mr-6 items-center">
                ...
                <div
                    ...
                    <form action="{{ route('inventaris.consumable.update-foto', $data->id) }}" method="post"
                        class="w-full h-full" enctype="multipart/form-data">
                        @csrf
                        <div class="relative w-full h-full" id="inputfilefoto">
                            @if ($data->foto_consumable == null)
                            ...
                            @else
                                <button type="button" id="updateImageButton"
                                    ...
                                    <img src="{{ asset('storage/foto_consumable/' . $data->foto_consumable) }}" alt="Preview"
                                        class="w-full h-full object-contain rounded-md" />
                                </button>
                            @endif
                            <input id="dropzone-file" type="file" accept="image/*" class="hidden" name="foto_consumable"
