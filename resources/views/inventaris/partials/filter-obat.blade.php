<form action="{{ route('inventaris.obat.filter') }}" method="post" id="form-filter">
    @csrf
    <div class="flex flex-col lg:flex-row justify-between">
        <div class="flex flex-wrap gap-1">
            <div class="space-y-1">
                <x-input-label for="nama_obat" class="mb-2">Nama Obat</x-input-label>
                <div class="relative">
                    <div class="absolute inset-y-0 start-0 flex items-center ps-2 pointer-events-none">
                        <span class="icon-[material-symbols--search] w-4 h-4 text-gray-500 dark:text-gray-400"></span>
                    </div>
                    <x-input-text type="search" id="nama_obat" name="nama_obat" class="p-2 ps-8"
            ...
            <div class="space-y-1">
                ...
                <select id="satuan_obat" name="satuan_obat"
                    ...
                    @foreach ($satuanObat as $satuan)
                        <option value="{{ $satuan['id'] }}" {{ old('satuanID') == $satuan['id'] ? 'selected' : '' }}>
                            {{ $satuan['nama_satuan'] }}
                        </option>
                    @endforeach
                </select>
...
<script>
    $(document).ready(function() {
        ...
        $('#nama_obat, #itemID, #satuan_obat').on('input', function() {
