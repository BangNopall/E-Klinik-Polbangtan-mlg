<div id="default-modal" tabindex="-1" aria-hidden="false"
    ...
    <div class="relative p-4 sm:p-5 w-full max-w-xl h-auto bg-white dark:bg-darker rounded-md shadow-md">
        ...
        <div class="relative">
            ...
            <!-- Modal body -->
            <form action="{{ route('inventaris.obat.update', $data->id) }}" method="post" id="additemform">
                @csrf
                <div class="space-y-2">
                    <div>
                        ...
                        <div x-data="selectConfigs()" x-init="fetchOptions()"
                            ...
                            <x-input-text @click.away="close()" x-model="filter"
                                ...
                                @keydown.arrow-down.prevent="focusNextOption()" type="search" id="satuan_obat"
                                name="satuan_obat" class="p-2" placeholder="Kategori"/>
                            <div x-show="isOpen()"
                                class="absolute shadow-lg bg-white dark:bg-darker top-100 z-40 w-full left-0 top-11 rounded max-h-[200px] overflow-y-auto">
