<x-modal name="tambah-item">
    <div class="relative p-4 sm:p-5 w-full max-w-2xl h-full md:h-auto">
        ...
        <div class="relative">
            ...
            <!-- Modal body -->
            <form action="{{ route('inventaris.obat.store') }}" method="post" id="additemform">
                @csrf
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    <div>
                        <x-input-label for="nama_obat" class="mb-2">Nama</x-input-label>
                        <x-input-text type="text" name="nama_obat" id="nama_obat" class="p-2.5"
                            placeholder="Nama Item" required="" value="{{ old('nama_obat') }}" />
                    </div>
                    <div>
                        <x-input-label for="satuan_id" class="mb-2">Kategori</x-input-label>
                        <div x-data="selectConfigs()" x-init="fetchOptions()"
                            ...
                            <x-input-text @click.away="close()" x-model="filter"
                                ...
                                @keydown.arrow-down.prevent="focusNextOption()" type="search" id="satuan_id"
                                name="nama_satuan" class="p-2" placeholder="Kategori" />
                            <div x-show="isOpen()"
                                        ...
                                        <div @click="onOptionClick(index)" :class="classOption(option.id, index)"
                                            ...
                                            <div
                                                ...
                                                <div class="w-full items-center flex">
                                                    <span x-text="option.nama_satuan"></span>
                                                </div>
                                            </div>
                                        </div>
                                    </template>
                        ...
                        <input type="text" id="satuanID" class="hidden" name="satuanID">
                    </div>
                    <div>
    ...
    <div id="url_satuan_obat" class="hidden"></div>
    <script>
        const url = '{{ route('api.getKategoriObat') }}';
        const element = document.getElementById('url_satuan_obat');
