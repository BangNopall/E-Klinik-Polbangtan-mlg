<x-app-layout>
    ...
    <div class="flex items-center justify-between px-2 sm:px-4 py-3 lg:py-5 border-b dark:border-blue-800">
        <h1 class="text-2xl font-semibold">Kamera QR</h1>
    </div>
    {{-- Main Content --}}
    <div class="px-2 sm:px-4 py-3 lg:py-5">
        <div
            ...
            <div class="text-center mb-3 mt-2 space-y-1">
                <div class="text-lg font-semibold text-gray-900 dark:text-white">Scan QR Medical</div>
                <p class="text-gray-500 dark:text-gray-300 text-sm text-center">Pindai kode QR untuk membuat surat
                    laporan.</p>
            </div>
            <div class="bg-gray-100 dark:bg-dark w-full md:w-[400px] mx-auto rounded-lg p-3 space-y-3">
                ...
                <div class="flex items-center ml-2">
                    ...
                    @else
                        <img src="{{ Storage::url('images/' . $user->avatar_url) }}" class="rounded-full w-9 h-9 md:w-12 md:h-12" id="fotoProfil"
                            alt="Foto Profil">
                    @endif
                    <div class="ml-3">
    ...
    <form action="{{ route('kesehatan.kamera.store') }}" method="post" id="form">
        @csrf
        <input type="hidden" name="token" id="token">
    </form>
    <script src="{{ asset('src/js/html5-qrcode.min.js') }}"></script>
    <script src="{{ asset('src/js/kesehatan/kamera.js') }}"></script>
</x-app-layout>
