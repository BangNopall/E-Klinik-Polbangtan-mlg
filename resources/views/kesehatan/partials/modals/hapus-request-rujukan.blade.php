...
<x-modal name="hapus-{{ $request->id }}">
    <div class="flex justify-center items-center relative p-4 w-full h-full md:h-auto">
        ...
        <div class="relative p-4 text-center sm:p-5 w-full">
            ...
            <span
                class="icon-[material-symbols--cancel-outline] text-gray-400 dark:text-gray-300 w-11 h-11 mb-3.5 mx-auto"></span>
            <p class="mb-4 text-gray-500 dark:text-gray-300">Apakah Anda yakin membatalkan request ini?</p>
            <form action="{{ route('kesehatan.reject-request-rujukan-konseling', $request->id) }}" method="POST">
                @csrf
                <div>
                    <x-input-label for="alasan" :value="'Alasan'" />
                    <textarea name="alasan" id="alasan" cols="4" rows="4" required
