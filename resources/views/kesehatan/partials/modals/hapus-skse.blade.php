<x-modal name="hapus-{{ $skse->nomor_surat }}">
    <div class="flex justify-center items-center relative p-4 w-full h-full md:h-auto">
        ...
        <div class="relative p-4 text-center sm:p-5 w-full">
            ...
            <p class="mb-4 text-gray-500 dark:text-gray-300 whitespace-normal">
                <strong>Peringatan:</strong> Tindakan ini akan menghapus surat
                <strong>{{ $skse->nomor_surat }}</strong> dari kesehatan beserta dokumen terkait.
            </p>
            <div class="flex justify-center items-center space-x-4">
                <x-secondary-button x-on:click="$dispatch('close')" type="button" class="py-2 px-3">
                    Batal
                </x-secondary-button>
                <form action="{{ route('kesehatan.form.surat-keterangan-sehat.delete', $skse->id) }}" method="POST">
                    @csrf
                    @method('DELETE')
