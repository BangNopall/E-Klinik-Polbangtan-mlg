{{-- hapus --}}
<x-modal name="hapus-{{ $dataPsikolog->id }}">
    <div class="flex justify-center items-center relative p-4 w-full h-full md:h-auto">
        ...
        <div class="relative p-4 text-center sm:p-5 w-full">
            ...
            <p class="mb-4 text-gray-500 dark:text-gray-300">Apakah Anda yakin menghapus riwayat konsultasi ini?</p>
            <div class="flex justify-center items-center space-x-4">
                <x-secondary-button x-on:click="$dispatch('close')" type="button" class="py-2 px-3">
                    Batal
                </x-secondary-button>
                <form action="{{ route('konseling.deleteKonsultasi',  $dataPsikolog->id) }}" method="POST">
                    @csrf
                    @method('DELETE')
