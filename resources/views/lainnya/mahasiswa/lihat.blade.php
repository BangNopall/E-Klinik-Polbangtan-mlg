        ...
        <div class="flex flex-col md:flex-row justify-center items-center sm:items-start gap-4">
            <div class="p-4 sm:p-8 bg-white dark:bg-darker sm:rounded-lg w-full md:w-auto">
                <div class="w-full md:max-w-xl">
                    @include('lainnya.mahasiswa.partials.profile-content')
                </div>
            </div>
            <div class="flex flex-col gap-4 w-full md:w-[800px]">
                @if ($user->dmti == true)
                {{-- @if ($user->dmti == true && isset($dmti)) --}}
                    <div class="p-4 sm:p-8 bg-white dark:bg-darker sm:rounded-lg">
                        <div class="w-full">
                            @include('lainnya.mahasiswa.partials.personal-content')
                        </div>
                    </div>
                @endif
                @if ($user->cdmi == true)
                {{-- @if ($user->cdmi == true && isset($cdmi)) --}}
                    <div class="p-4 sm:p-8 bg-white dark:bg-darker sm:rounded-lg">
                        <div class="w-full">
                            @include('lainnya.mahasiswa.partials.mahasiswa-content')
                        </div>
                    </div>
                @endif
                <div class="p-4 sm:p-8 bg-white dark:bg-darker sm:rounded-lg">
                    <div class="w-full">
                        @include('lainnya.mahasiswa.partials.rpd-content')
                    </div>
                </div>
                <div class="p-4 sm:p-8 bg-white dark:bg-darker sm:rounded-lg">
                    <div class="w-full">
                        @include('lainnya.mahasiswa.partials.password-content')
                    </div>
                </div>
            </div>
        </div>
